...
function AccessPage() {
	...
	if (PHPSessionID) {
		if (PageName == "" || PageName == "index.php") {
			...
			} else if (getCookie("user") == 1) {
				window.location.replace("accountant_board.php");
			} else {
	...
	switch (api) {
		case "multi_view_all_feesheets":
			...
			content.forEach(element => {
				...
				element['add_date'] = convertDate(new Date(Date.parse(element['add_date'])));
				...
				element["view"] = `<button class="btn" onclick="EditFeeSheet_admin(${i})"><i class="fa-solid fa-pen"></i></button>`;
